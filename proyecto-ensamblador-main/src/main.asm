...
SEL1_L  equ $-SEL1
SEL2    db 10,'SELECCIONADO: PONG',10
SEL2_L  equ $-SEL2
...
.wait:
    ...
    mov bl, al           ; <--- GUARDAMOS la tecla en BL para no perderla
    cmp bl, 'Q'          ; Comparamos usando BL
    je .esc
    cmp bl, 'E'
    je .enter

    mov eax, [selected]  ; Ahora EAX tiene 0, 1 o 2; BL sigue teniendo 'L' o 'R'

    cmp bl, 'L'          ; ¿Pulsaste Izquierda?
    jne .chkR
    cmp eax, 0
    je .wait             ; Si ya es 0, no hacemos nada, volvemos a esperar
    dec eax
    mov [selected], eax
    jmp .redraw          ; Redibujamos para ver el cambio

.chkR:
    cmp bl, 'R'          ; ¿Pulsaste Derecha?
    jne .wait
    cmp eax, 2
    je .wait
    inc eax
